...
mod tests {
    use super::{
        ...
        format_tool_result, format_turn_duration, format_ultraplan_report,
        format_unknown_slash_command, format_unknown_slash_command_message,
        format_user_visible_api_error, is_repl_recoverable_kind, merge_prompt_with_stdin,
        ...
        summarize_tool_payload_for_markdown, try_recover_repl_turn_error,
        try_resolve_bare_skill_prompt, validate_model_syntax, validate_no_args,
        write_mcp_server_fixture, write_turn_footer, CliAction, CliOutputFormat, CliToolExecutor,
    ...
    #[test]
    fn default_permission_mode_falls_back_to_read_only_when_nothing_set() {
        let _guard = env_lock();
        let root = temp_dir();
        let cwd = root.join("project");
        let config_home = root.join("config-home");
    ...
    fn mcp_serve_policy_denies_bash_under_empty_config() {
        ...
        let outcome = with_current_dir(&cwd, || {
            ...
            let policy = super::permission_policy(permission_mode, &feature_config, &tool_registry)
...
"#,
    ...
    fn try_recover_repl_turn_error_propagates_fatal_classes() {
        ...
        let propagated = try_recover_repl_turn_error(error).expect("fatal classes must propagate");
        assert!(propagated
            .to_string()
            .contains("missing Anthropic credentials"));
